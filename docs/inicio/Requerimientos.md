...
**Fecha:** Marzo 2026  
**Versión:** 2.0 (actualizada — refleja implementación real)
...
| RF-02 | Validación de prerrequisitos | **Must** | Dado un estudiante con un historial de cursos aprobados, el sistema debe validar en **<500ms** que todos los cursos seleccionados cumplen sus prerrequisitos, además de verificar corequisitos, límite de créditos y análisis de sobrecarga académica. | Se rechaza matrícula que incluya "Base de Datos Avanzada" sin haber aprobado "Base de Datos I". | 2 |
| RF-03 | Validación de créditos | **Must** | El sistema debe calcular la suma de créditos de los cursos seleccionados por un estudiante y verificar que esté en el rango configurable **[minCredits, maxCredits]** (default: 12-25), con reducción a 18 si tiene >2 cursos desaprobados, mostrando mensaje claro si está fuera. | Estudiante con matrícula de 26 créditos ve mensaje "Excede el límite de 25 créditos por semestre". | 2 |
| RF-04 | Generación CSP | **Must** | El motor debe producir un horario **sin conflictos** (0 solapamientos de docentes, aulas y estudiantes) para un conjunto de 50 cursos y 100 estudiantes en **≤30 segundos**, usando backtracking + MRV + Forward Checking + scoring 4D. | Se ejecuta con datos de prueba y se verifica automáticamente que no hay dos cursos asignados a la misma aula/hora. | 3 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| RNF-01 | Rendimiento / Eficiencia | **Must** | El endpoint `POST /api/generations/generate` (CSP) y `POST /api/student-schedule/generate` (Kuhn) deben responder en **≤30 segundos** para el escenario máximo (50 cursos, 100 estudiantes) en hardware estándar (CPU 2.5 GHz, 8GB RAM). | Tiempo p95 < 30s en 10 ejecuciones consecutivas. | Artilley.io o k6 con prueba de carga. |
...
| RNF-05 | Fiabilidad / Robustez | **Should** | El motor CSP no debe fallar (crash) ante entradas inválidas (ej. restricciones imposibles). Debe retornar código HTTP 400 con mensaje descriptivo. El matching estudiante debe reportar cursos sin cobertura como `uncoveredCourses` en lugar de fallar. | Tasa de fallos no controlados < 1% en 100 ejecuciones con datos aleatorios. | Pruebas de mutación (Stryker). |
